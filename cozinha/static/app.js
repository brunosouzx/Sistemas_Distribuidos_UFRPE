...
function setupEventListeners() {
  ...
  window.onclick = function (event) {
    if (elements.modalAcao && event.target === elements.modalAcao)
      fecharModal();
    if (elements.modalCancelar && event.target === elements.modalCancelar)
      fecharModalCancelar();
    if (elements.modalEstoque && event.target === elements.modalEstoque)
      if (typeof fecharModalEstoque === "function") fecharModalEstoque();
  };
}
...
function criarPedidoCard(pedido) {
  ...
  if (pedido.status === "CANCELADO") {
    ...
    card.innerHTML = `
            ...
            <div class="pedido-info">
                ...
                <p class="item-destaque" style="text-decoration: line-through; color: #999;">🍔 ${
                  pedido.item
                }</p>
                <div class="pedido-obs" style="background: #fadbd8; border-color: #e74c3c; color: #78281f;">
                    🚫 Motivo: ${motivo}
                </div>
                <p class="pedido-tempo">📅 ${new Date(
  ...
  if (pedido.status === "PREPARANDO" && pedido.data_inicio_preparo) {
    const horaInicio = new Date(pedido.data_inicio_preparo).toLocaleTimeString(
      "pt-BR",
...
function atualizarEstatisticas() {
  elements.statRecebidos.textContent = state.pedidos.filter(
    (p) => p.status === "RECEBIDO"
  ).length;
  elements.statPreparando.textContent = state.pedidos.filter(
    (p) => p.status === "PREPARANDO"
  ).length;
  elements.statProntos.textContent = state.pedidos.filter(
    (p) => p.status === "PRONTO"
  ).length;
  if (elements.statCancelados) {
    elements.statCancelados.textContent = state.pedidos.filter(
  ...
  elements.estoqueTableBody.innerHTML = state.estoque
    .map((item) => {
      ...
      let statusClass =
        qtd === 0
          ? "status-critico"
          : qtd <= item.estoque_minimo
          ? "status-baixo"
          : "status-ok";
      let statusText =
        qtd === 0 ? "Esgotado" : qtd <= item.estoque_minimo ? "Baixo" : "OK";
      return `
        <tr>
          ...
          <td><button class="btn-sm btn-success" onclick="abrirModalEstoque('${
            item.nome
          }')">+ Repor</button></td>
        </tr>
      `;
...
function abrirModalIniciar(id) {
  ...
  if (elements.tempoPreparoContainer)
...
window.addEventListener("beforeunload", () => {
  if (state.atualizacaoAutomatica) clearInterval(state.atualizacaoAutomatica);
});
